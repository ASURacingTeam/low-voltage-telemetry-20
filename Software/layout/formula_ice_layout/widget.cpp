    ...
    , ui(new Ui::Widget)
{
    ...
    gearval = 0;
    steeringval = 0;
    gear = view->rootObject();
    gear->setProperty("gear",gearval);
   ...
   // m->setPostion(420,60);
    m->setType("Speed");
    //m2->setPostion(730,60);
    m2->setType("RPM");

    title = new QLabel("Formula 2020 ICE Car");
...
void Widget::serial_receive()
{
    ...
    if(list.length()== 8)
    {
        ...
        m2->update_values(list[3].toInt(&ok,16));
        speedPlot->updateValue(list[4].toInt(&ok,16));
        vbar->update_voltage(list[5].toInt(&ok,16));
        gear->setProperty("gear",list[6].toInt(&ok,16));
        steering->setProperty("steering_angle",list[7].toInt(&ok,16));
